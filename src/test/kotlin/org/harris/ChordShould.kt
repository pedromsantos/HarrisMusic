...
class ChordShould {
    ...
    fun `Return bass note`() {
        assertThat(Chord(Major, C).bass(), equalTo(C))
    }

    @Test
    fun `Return lead note`() {
        assertThat(Chord(Major, C).lead(), equalTo(G))
    }

    @Test
    fun `be named after the root note`() {
        assertThat(Chord(Major, C).name(), startsWith("C"))
    }

    @Test
    fun `be named after the root note and the quality`() {
        assertThat(Chord(Major, C).name(), equalTo("CMajor"))
    }

    @Test
    fun `return note for function root`() {
        assertThat(Chord(Major, C).noteForFunction(Root), equalTo(C))
    }

    @Test
    fun `return note for function third`() {
        assertThat(Chord(Major, C).noteForFunction(Third), equalTo(E))
    }

    @Test
    fun `return note for function fifth`() {
        assertThat(Chord(Major, C).noteForFunction(Fifth), equalTo(G))
    }
}
